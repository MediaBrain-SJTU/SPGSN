...
def main(opt):
    ...
    checkpoint_dir = '3D_dct{:d}_L{:d}_J{:d}_Wpg{:d}_Wp{:d}'.format(opt.dct_n, opt.num_stage, opt.J, 4, 2)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        # per epoch
        lr_now, t_l = train(train_loader, model, optimizer, lr_now=lr_now, max_norm=opt.max_norm, is_cuda=is_cuda, dim_used=train_dataset.dim_used, dct_n=dct_n)
...
def val(train_loader, model, is_cuda=False, dim_used=[], dct_n=15):
    ...
    with torch.no_grad():
        for i, (inputs, targets, all_seq) in enumerate(train_loader):
            ...
            if i%100==0:
                print('{}/{}|TrainLoss {:.4f}, batch time {:.4f}s|total time{:.2f}s'\
